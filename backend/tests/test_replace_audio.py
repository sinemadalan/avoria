...
class WritingReplacer:
    def __init__(self, error: Exception | None = None) -> None:
        self.error = error
        self.calls: list[tuple[Path, Path, Path, object]] = []

    def resolve_profile(self, target: Path, audio: Path) -> ReplaceAudioProfile:
        return _profile(duration=2)

    def replace(self, target: Path, audio: Path, output: Path, profile: object) -> None:
        self.calls.append((target, audio, output, profile))
...
def test_celery_dispatches_standalone_replace_audio_service(
    ...
    try:
        process_media_job.run(video_id, "replace_audio", {"audio_media_id": audio_id})
    finally:
        process_media_job.pop_request()

    assert captured["audio_replacer"] is replacer
    assert captured["replace_audio"] == ReplaceAudioSpec(audio_id)
...
def test_real_replace_removes_all_original_audio_and_stream_copies_video(
    ...
    _make_audio(ffmpeg, external, 3, 880)
    ...
    samples = _pcm(ffmpeg, output, 0.25, 1)
...
def test_real_short_audio_is_padded_with_silence_and_never_looped(tmp_path: Path) -> None:
    ffmpeg, ffprobe = _tools()
    target, external, output = tmp_path / "target.mp4", tmp_path / "external.wav", tmp_path / "output.part.mp4"
    _make_video(ffmpeg, target, 6, "libx264", "mp4")
    ...
    head, middle, tail = _pcm(ffmpeg, output, 0.25, 1), _pcm(ffmpeg, output, 2.5, 1), _pcm(ffmpeg, output, 4.5, 1)
    assert inspection.format.duration_seconds == pytest.approx(6, abs=0.2)
    assert float(audio_stream["duration"]) == pytest.approx(6, abs=0.2)
